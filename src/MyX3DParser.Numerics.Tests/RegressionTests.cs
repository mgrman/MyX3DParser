...
namespace MyX3DParser.Tests
{
    public class RegressionTests
    {
        ...
        [Theory]
        [MemberData(nameof(RegressionData))]
        public void Gltf_Regression(string relativePath)
        {
            var absPath = Path.Combine(regressionDataFolder, relativePath);

            var resultGlb = ConvertToGLTF.ConvertToGLB(File.ReadAllText(absPath));

            var expectedFileName = Path.GetFileNameWithoutExtension(absPath) + $".glb";
            var expectedFilePath = Path.Combine(Path.GetDirectoryName(absPath) ?? "", expectedFileName);
            if (!File.Exists(expectedFilePath))
            {
                File.WriteAllBytes(GetWrongPath(expectedFilePath), resultGlb);
                throw new Exception($"Missing results file '{expectedFileName}'!");
            }
            var expectedGlb = File.ReadAllBytes(expectedFilePath);
            try
            {
                Assert.Equal(expectedGlb, resultGlb);
        }
